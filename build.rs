...
fn fetch(version: &str, into: &Path) {
    let url =
        format!("https://github.com/ancaria-dev/coderpack/releases/download/v{version}/agent.zip");
    ...
    let unpacked = run(&[
        ...
        (
            ...
            vec![
                "-NoProfile",
                "-Command",
                &format!("Expand-Archive -Path '{target}' -DestinationPath '{folder}' -Force"),
            ],
        ),
    ]);
...
fn embed(dir: &Path, origin: &str, out: &Path) {
    ...
    fs::write(staged.join("addr.js"), js::compact(&read(&addr))).expect("could not stage addr.js");
    ...
    let mut table =
        String::from("// Generated by build.rs. The agent, minified, one entry per module.\n");
