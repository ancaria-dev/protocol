...
unsafe extern "C" fn call_on_message<I: ScriptHandler>(
    ...
    match formatted_msg {
        ...
        _ => {
            // PATCHED (see vendor/README.md): upstream casts user_data, which
            // is a *mut CallbackHandler, to *mut I -- the caller's handler type.
            // That reinterprets one struct's memory as another's and faults on
            // the first non-RPC message.  The handler actually lives in
            // CallbackHandler::script_handler, put there by add_handler.
            let callback_handler: &mut CallbackHandler =
                &mut *(user_data as *mut CallbackHandler);
